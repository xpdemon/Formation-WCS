...
public class BeerApi {
    ...
    public static void main(String[] args){
        ...
        for ( int i = 0 ; i< allBeers.getAllBeers().size();i++) {
            ...
            System.out.println(allBeers.getAllBeers().get(i).ingredient.get(j).type +" : "+ allBeers.getAllBeers().get(i).ingredient.get(j).name +" ; "+allBeers.getAllBeers().get(i).ingredient.get(j).type +" : "+ allBeers.getAllBeers().get(i).ingredient.get(j).quantities);
